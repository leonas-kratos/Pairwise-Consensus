# -*- coding: utf-8 -*-
"""
PC_EKF_RT.py — PC-EKF-v3 Realtime (Numba JIT)
===============================================
Đọc /dev/serial/by-id/... @ 921600, in X Y ra console, ghi CSV.

Thuật toán: EKF + Pairwise Consensus V3 (MAD auto-normalized T-kernel → adaptive R).
...
Q = 0.01
R = 50.0
PC_R_SCALE = 30.0   # tuned for EKF (PC-EKFV3)
LOG_FILE   = "PC_EKF_realtime_log.csv"
TAG = "PC_EKF"
...
@njit(cache=True)
def _pc_ekf_step_nb(x, P, z_slant, anchors, anchor_h, Q, r_base, r_scale):
    N = anchors.shape[0]
    P = P + np.eye(2) * Q
    ...
    S = (H @ P) @ H.T
    for i in range(N):
        S[i, i] += r_base * (1.0 + r_scale * (1.0 - scores[i]))
...
def _step(x, P, z, state, r_scale=None):
    ...
    x, P = _pc_ekf_step_nb(x, P, z, ANCHORS, ANCHOR_HEIGHT, Q, R, r_scale)
    return x, P, state

def _warmup():
    print("[PC_EKF_RT] Warming up JIT kernels...", end=" ", flush=True)
    _z = np.array([1000.0, 1200.0, 800.0, 900.0])
    _ls_position_nb(np.array([slant_to_ground_py(d) for d in _z]), ANCHORS)
    _pc_ekf_step_nb(np.array([3000.0,2000.0]), np.eye(2)*1e6, _z, ANCHORS, ANCHOR_HEIGHT,
                    Q, R, PC_R_SCALE)
    print("OK")
...
def main():
    ap = argparse.ArgumentParser(description="PC_EKF Realtime — Numba JIT")
    ...
    args = ap.parse_args()
    print("PC_EKF_RT — Pairwise Consensus EKF v3 (Numba JIT)")
