# -*- coding: utf-8 -*-
"""
Huber_UKF_RT.py — Huber-UKF Realtime (Numba JIT)
===============================================
Đọc /dev/serial/by-id/... @ 921600, in X Y ra console, ghi CSV.

Thuật toán: UKF + Huber IRLS M-estimator (reweight R theo residual chuẩn hóa).
...
HUBER_DELTA   = 20.0
HUBER_MAXITER = 5
LOG_FILE      = "Huber_UKF_realtime_log.csv"
TAG = "Huber_UKF"
...
@njit(cache=True)
def _huber_ukf_step_nb(x, P, z_slant, anchors, anchor_h, Q, R_base, delta, maxiter, Wm, Wc, c):
    N = anchors.shape[0]
    x_pred = x.copy()
    P_pred = P + np.eye(2) * Q
    ...
    for _ in range(maxiter):
        for i in range(N):
            ...
            if pzz_ii < 1e-9: pzz_ii = 1e-9
            r_sc = (z_slant[i] - z_hat[i]) / math.sqrt(pzz_ii)
            ar = abs(r_sc)
...
def _step(x, P, z, state, delta=None, maxiter=None):
    if delta is None: delta = HUBER_DELTA
    if maxiter is None: maxiter = HUBER_MAXITER
    x, P = _huber_ukf_step_nb(x, P, z, ANCHORS, ANCHOR_HEIGHT, Q, R, delta, maxiter, WM, WC, C_SPREAD)
    return x, P, state

def _warmup():
    print("[Huber_UKF_RT] Warming up JIT kernels...", end=" ", flush=True)
    _z = np.array([1000.0, 1200.0, 800.0, 900.0])
    _ls_position_nb(np.array([slant_to_ground_py(d) for d in _z]), ANCHORS)
    _huber_ukf_step_nb(np.array([3000.0,2000.0]), np.eye(2)*1e6, _z, ANCHORS, ANCHOR_HEIGHT,
                       Q, R, HUBER_DELTA, HUBER_MAXITER, WM, WC, C_SPREAD)
    print("OK")
...
def main():
    ap = argparse.ArgumentParser(description="Huber_UKF Realtime — Numba JIT")
    ...
    args = ap.parse_args()
    print("Huber_UKF_RT — Huber IRLS UKF (Numba JIT)")
    ...
    kw = dict(delta=args.delta, maxiter=args.maxiter)
