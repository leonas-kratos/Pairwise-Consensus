# -*- coding: utf-8 -*-
"""
MCC_UKF_RT.py — MCC-UKF Realtime (Numba JIT)
===============================================
Đọc /dev/serial/by-id/... @ 921600, in X Y ra console, ghi CSV.

Thuật toán: UKF + Maximum Correntropy Criterion (Gaussian kernel reweight R).
...
MCC_MAXITER   = 5
LOG_FILE      = "MCC_UKF_realtime_log.csv"
TAG = "MCC_UKF"
...
@njit(cache=True)
def _mcc_ukf_step_nb(x, P, z_slant, anchors, anchor_h, Q, R_base, kernel_bw, maxiter, Wm, Wc, c):
    N = anchors.shape[0]
    x_pred = x.copy()
    P_pred = P + np.eye(2) * Q
    ...
    for _ in range(maxiter):
        ...
        dx0 = x_new[0] - x_cur[0]; dx1 = x_new[1] - x_cur[1]
...
def _step(x, P, z, state, kernel_bw=None, maxiter=None):
    if kernel_bw is None: kernel_bw = MCC_KERNEL_BW
    if maxiter is None: maxiter = MCC_MAXITER
    x, P = _mcc_ukf_step_nb(x, P, z, ANCHORS, ANCHOR_HEIGHT, Q, R, kernel_bw, maxiter, WM, WC, C_SPREAD)
    return x, P, state

def _warmup():
    print("[MCC_UKF_RT] Warming up JIT kernels...", end=" ", flush=True)
    _z = np.array([1000.0, 1200.0, 800.0, 900.0])
    _ls_position_nb(np.array([slant_to_ground_py(d) for d in _z]), ANCHORS)
    _mcc_ukf_step_nb(np.array([3000.0,2000.0]), np.eye(2)*1e6, _z, ANCHORS, ANCHOR_HEIGHT,
                     Q, R, MCC_KERNEL_BW, MCC_MAXITER, WM, WC, C_SPREAD)
    print("OK")
...
def main():
    ap = argparse.ArgumentParser(description="MCC_UKF Realtime — Numba JIT")
    ...
    ap.add_argument("--kernel-bw", type=float, default=MCC_KERNEL_BW)
    ap.add_argument("--maxiter", type=int, default=MCC_MAXITER)
    args = ap.parse_args()
    print("MCC_UKF_RT — Maximum Correntropy Criterion UKF (Numba JIT)")
    print(f"  Q={Q}  R={R}  (hardcoded)  kernel_bw={args.kernel_bw}  maxiter={args.maxiter}")
    print(f"  Anchors: {[hex(i) for i in ANCHOR_IDS]}")
    print(f"  Height : {ANCHOR_HEIGHT} mm\n")
    kw = dict(kernel_bw=args.kernel_bw, maxiter=args.maxiter)
