# -*- coding: utf-8 -*-
"""
MCC_EKF_RT.py — MCC-EKF Realtime (Numba JIT)
===============================================
Đọc /dev/serial/by-id/... @ 921600, in X Y ra console, ghi CSV.

Thuật toán: EKF + Maximum Correntropy Criterion (Gaussian kernel reweight R).
...
MCC_MAXITER   = 5
LOG_FILE      = "MCC_EKF_realtime_log.csv"
TAG = "MCC_EKF"
...
@njit(cache=True)
def _mcc_ekf_step_nb(x, P, z_slant, anchors, anchor_h, Q, R_base, kernel_bw, maxiter):
    N = anchors.shape[0]
    P_pred = P + np.eye(2) * Q
    ...
    for _ in range(maxiter):
        ...
        if math.sqrt(dx0*dx0 + dx1*dx1) < 1e-3:
            ...
            break
        x_cur = x_new
    P_new = _make_spd_nb((np.eye(2) - K @ H) @ P_pred)
    return x_cur, P_new
...
def _step(x, P, z, state, kernel_bw=None, maxiter=None):
    ...
    x, P = _mcc_ekf_step_nb(x, P, z, ANCHORS, ANCHOR_HEIGHT, Q, R, kernel_bw, maxiter)
    return x, P, state

def _warmup():
    print("[MCC_EKF_RT] Warming up JIT kernels...", end=" ", flush=True)
    _z = np.array([1000.0, 1200.0, 800.0, 900.0])
    _ls_position_nb(np.array([slant_to_ground_py(d) for d in _z]), ANCHORS)
    _mcc_ekf_step_nb(np.array([3000.0,2000.0]), np.eye(2)*1e6, _z, ANCHORS, ANCHOR_HEIGHT,
                     Q, R, MCC_KERNEL_BW, MCC_MAXITER)
    print("OK")
...
def main():
    ap = argparse.ArgumentParser(description="MCC_EKF Realtime — Numba JIT")
    ...
    args = ap.parse_args()
    print("MCC_EKF_RT — Maximum Correntropy Criterion EKF (Numba JIT)")
