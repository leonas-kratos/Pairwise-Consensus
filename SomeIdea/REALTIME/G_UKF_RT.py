# -*- coding: utf-8 -*-
"""
G_UKF_RT.py — G-UKF Realtime (Numba JIT)
===============================================
Đọc /dev/serial/by-id/... @ 921600, in X Y ra console, ghi CSV.

Thuật toán: Gaussian-smoothed UKF (pre-filter measurements bằng kernel Gaussian).
...
G_SIGMA  = 5.0
G_N_HALF = 4
LOG_FILE = "G_UKF_realtime_log.csv"
TAG = "G_UKF"
...
@njit(cache=True)
def _g_ukf_step_nb(x, P, z_slant, anchors, anchor_h, Q, R, buf, buf_len, kernel, win, Wm, Wc, c):
    N = anchors.shape[0]
    if buf_len < win:
        for j in range(N):
...
def _step(x, P, z, state, sigma=None, n_half=None, win=None):
    x, P, buf, buf_len = _g_ukf_step_nb(
        x, P, z, ANCHORS, ANCHOR_HEIGHT, Q, R,
        state['buf'], state['buf_len'], state['kernel'], state['win'], WM, WC, C_SPREAD)
    state['buf'] = buf
    state['buf_len'] = buf_len
    return x, P, state

def _warmup():
    print("[G_UKF_RT] Warming up JIT kernels...", end=" ", flush=True)
    _z = np.array([1000.0, 1200.0, 800.0, 900.0])
    _ls_position_nb(np.array([slant_to_ground_py(d) for d in _z]), ANCHORS)
    st = _make_state()
    _g_ukf_step_nb(np.array([3000.0,2000.0]), np.eye(2)*1e6, _z, ANCHORS, ANCHOR_HEIGHT,
                   Q, R, st['buf'], 0, KERNEL, WIN, WM, WC, C_SPREAD)
    print("OK")
...
def main():
    ap = argparse.ArgumentParser(description="G_UKF Realtime — Numba JIT")
    ...
    args = ap.parse_args()
    print("G_UKF_RT — Gaussian-smoothed UKF (Numba JIT)")
