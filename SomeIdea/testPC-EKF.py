...
ANCHORS = np.array([
    [0,    0   ],   # anchor 0x1001
    [0,    4720],   # anchor 0x1002
    [6300, 4720],   # anchor 0x1003
    [6300, 0   ],   # anchor 0x1004
], dtype=float)
...
def parse_file(path):
    """
    Format mới: mỗi dòng 1 anchor đo
      timestamp_s, anchor_id, distance_mm, v_mm_s, heading_deg

    anchor_id: 0x1001→idx 0, 0x1002→idx 1, 0x1003→idx 2, 0x1004→idx 3
    Group theo timestamp, lấy đủ 4 anchor mới tạo 1 timestep.
    v_mm_s và heading_deg có nhưng chưa dùng.
    """
    ANCHOR_ID_MAP = {0x1001: 0, 0x1002: 1, 0x1003: 2, 0x1004: 3}

    # Đọc tất cả dòng hợp lệ
    raw_records = []
    with open(path, 'r') as f:
        for line in f:
            ...
            try:
                ts         = float(parts[0])
                anchor_raw = parts[1]
                # Hỗ trợ cả "0x1001" (hex string) và số nguyên
                anchor_id  = int(anchor_raw, 16) if anchor_raw.startswith('0x') \
                             else int(anchor_raw)
            ...
            if anchor_id not in ANCHOR_ID_MAP:
                continue
            raw_records.append((ts, ANCHOR_ID_MAP[anchor_id], dist_mm))

    if not raw_records:
        return None

    # Mỗi anchor đo ở timestamp riêng → group theo cụm liên tiếp.
    # Chiến lược: quét tuần tự, mở "slot" mới khi gặp anchor_id đã có
    # trong slot hiện tại (tức là vòng đo mới bắt đầu).
    rows = []
    slot = [None] * N_ANCHORS   # slot hiện tại

    for ts, idx, dist in raw_records:
        if slot[idx] is not None:
            # anchor này đã có trong slot → flush slot cũ, mở slot mới
            if all(v is not None for v in slot):
                d_ground = [slant_to_ground(v) for v in slot]
    ...
    # Flush slot cuối
    if all(v is not None for v in slot):
        rows.append([slant_to_ground(v) for v in slot])
    ...
    return {
        'dist': data,                          # (T, 4) — ground distances
        'v'   : np.zeros(len(data)),           # placeholder (chưa dùng)
        'gz'  : np.zeros(len(data)),           # placeholder (chưa dùng)
    }
