...
SO SÁNH 3 PHƯƠNG PHÁP:
  1. Raw + LS
  2. KF + LS         (fixed Q, R)
  3. PC-KF-v3 + LS   (MAD auto-normalized, sigma-free)
...
PCKF_Q       = 0.01
PCKF_R_BASE  = 50.0
PCKF_R_SCALE = 100.0   # ← PARAM DUY NHẤT CẦN TUNE (thử 2 → 30)
...
class PCKF_v3:
    ...
    Pipeline mỗi timestep:
      ...
      8. KF update: K[i] = P_pred[i] / (P_pred[i] + R_adaptive[i])
                    x̂[i] += K[i] * ν[i]
                    P[i]  = (1 - K[i]) * P_pred[i]
      9. LS position từ filtered distances
    """
...
def grid_search(val_files, gt_xy):
    ...
    grid = {
        'q'       : [1.0],
        'r_base'  : [1.0],
        'r_scale' : [2.0, 5.0, 10.0, 15.0, 20.0, 30.0, 40.0, 50.0, 100.0],
    }
    keys   = list(grid.keys())
    ...
    for rmse, p in results[:5]:
        print(f"  {rmse:>9.1f}mm {p['q']:>8.3f} {p['r_base']:>8.1f}"
              f" {p['r_scale']:>8.1f}")
...
def main():
    ...
    err, positions, per_file_rmse = evaluate_files(
        eval_files, gt_xy,
